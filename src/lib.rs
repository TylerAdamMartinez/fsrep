...
use colored::*;
use regex::Regex;
use std::{error::Error, fmt::Display, fs, process, thread, thread::JoinHandle};
...
impl Config {
    pub fn new(mut system_args: std::env::Args) -> Result<Config, &'static str> {
        ...
        match system_args.next() {
            ...
            None => {
                return Err(
                    "insufficient amount of passed arguments: needs at least one file argument",
                )
            }
        };
...
pub fn run_process(program_config: Config) -> Result<(), Box<dyn Error + 'static>> {
    ...
    for filename in program_config.filenames.into_iter() {
        ...
        thread_join_handler.push(thread::spawn(move || {
            let file_contents: String =
                fs::read_to_string(&filename).unwrap_or_else(|error_flag| {
                    fsrep_failure(error_flag, Some(&filename));
                    process::exit(1);
                });
            let regex_query_results: Vec<SearchResults> =
                search(&regex_query_expression_clone, file_contents);
            print_results(
                &filename,
                &regex_query_expression_clone,
                &regex_query_results,
            );
        }));
    }
    ...
    for thread_join_handle in thread_join_handler {
        thread_join_handle.join().expect(&format!(
            "{}{}",
            "fsrep failure".red().bold(),
            ": thread panic"
        ));
    }
    Ok(())
...
fn print_results(
    filename: &String,
    regex_query_expression: &Regex,
    regex_query_results: &Vec<SearchResults>,
) {
    let fsrep_success_msg = "fsrep success".green().bold();
    let number_query_of_results = regex_query_results.len().to_string().green().bold();
    println!(
        "{}: In file: '{}' {} matches found",
        fsrep_success_msg, filename, number_query_of_results
    );

    for result in regex_query_results.iter() {
        let colorised_result_line_content =
            highlight_regex_result(&regex_query_expression, &result.line_content);
        println!(
            "{}: {}",
            result.line_number.to_string().cyan().bold(),
            colorised_result_line_content
        );
    }
}
...
fn highlight_regex_result(regex_query_expression: &Regex, line_content: &String) -> String {
    ...
    if query_match.start() == 0 {
        let color_match = &line_content[query_match.start()..query_match.end()]
            .green()
            .bold();
